...
const Complaint = () => {
  ...
  const submit = (e) => {
    ...
    if(fileUrl !== ''){
      ...
      data["media"] = fileUrl;
      data["user_id"] = localStorage.getItem('loggedIn_user_id') != null 
                                      ? localStorage.getItem('loggedIn_user_id') : 'ritik_sharma';
      console.log(data);

      supabase.from('complaintDetails').insert(data)
      .then((res) => {
        console.log(res);
      })
    }
    ...
    supabase.storage
      ...
      .then((res) => {
        ...
        if (res && res.data && res.data.path) {
          ...
          setLoading(false);
          setFileUrl(supabase.storage.from("mediabuckets").getPublicUrl(res.data.path).data.publicUrl);
        } 
      }).catch((err) => {
